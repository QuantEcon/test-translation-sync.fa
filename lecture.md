...
translation:
  ...
  headings:
    ...
    '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': استفاده از `plt.subplot()` از `matplotlib` برای چندین نمودار
    Summary: خلاصه
---
...
در مقابل، `np.linspace()` آرایه‌هایی با تعداد نقاط مشخص ایجاد می‌کند:
...
*Matplotlib* کتابخانه پایه ترسیم نمودار برای پایتون است. این کتابخانه کنترل دقیقی بر ظاهر نمودار فراهم می‌کند.
...
### استفاده از `plt.subplot()` از `matplotlib` برای چندین نمودار
...
ابزارهای برنامه‌نویسی مانند **numpy**، *pandas*، و `matplotlib` برای پژوهش‌های اقتصادی مدرن ضروری هستند. بر این کتابخانه‌ها تسلط یابید تا بتوانید داده‌ها را به‌طور کارآمد تحلیل کرده و نتایج را تجسم‌سازی کنید.
