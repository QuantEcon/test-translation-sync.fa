...
heading-map:
  ...
  Questions & Answers: سؤال و جواب
  '"Edge Cases" and [Special] {Characters}': «موارد خاص» و [ویژه] {کاراکترها}
  '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': استفاده از `plt.subplot()` در `matplotlib` برای چندین نمودار
  Summary: خلاصه
---
...
کتابخانه `numpy` عملیات آرایه‌ای کارآمدی برای محاسبات عددی فراهم می‌کند. آرایه‌ها پایه و اساس محاسبات علمی در Python هستند.

آرایه‌های NumPy از عملیات برداری‌سازی پشتیبانی می‌کنند و از حلقه‌های کُند Python اجتناب می‌نمایند:
...
کتابخانه **pandas** قابلیت‌های قدرتمندی برای دستکاری داده فراهم می‌کند. DataFrameها ساختارهای داده دوبعدی برچسب‌دار هستند.
...
- **س**: کدام سریع‌تر است: `numpy` یا Python معمولی؟
- **ج**: `numpy` معمولاً برای عملیات عددی ۱۰ تا ۱۰۰ برابر سریع‌تر است

- **س**: آیا می‌توانم از `pandas` با مجموعه داده‌های بزرگ استفاده کنم؟
- **ج**: بله، اما برای مجموعه داده‌های بزرگ‌تر از حافظه RAM، استفاده از `dask` را در نظر بگیرید

## «موارد خاص» و [ویژه] {کاراکترها}

این بخش کاراکترهای ویژه مختلف در عناوین را آزمایش می‌کند.

### استفاده از `plt.subplot()` در `matplotlib` برای چندین نمودار
...
ابزارهای برنامه‌نویسی مانند **numpy**، *pandas* و `matplotlib` برای پژوهش اقتصادی مدرن ضروری هستند. تسلط بر این کتابخانه‌ها برای تحلیل کارآمد داده‌ها و مصورسازی نتایج الزامی است.
