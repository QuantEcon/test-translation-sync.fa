...
translation:
  ...
  headings:
    ...
    'Working with **Pandas DataFrames**::Reading Data: `pd.read_csv()` vs `pd.read_excel()`': 'خواندن داده‌ها: `pd.read_csv()` در برابر `pd.read_excel()`'
    ...
    'Special Cases: $\LaTeX$ in Headings': 'موارد خاص: $\LaTeX$ در تیترها'
    ...
    '"Edge Cases" and [Special] {Characters}': '"موارد لبه" و [ویژه] {نویسه‌ها}'
    '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': استفاده از `plt.subplot()` در `matplotlib` برای رسم چند نمودار
    Summary: خلاصه
---
...
کتابخانه `numpy` عملیات آرایه‌ای کارآمدی را برای محاسبات عددی فراهم می‌کند. آرایه‌ها اساس محاسبات علمی در پایتون هستند.
...
روش‌های مختلف ایجاد آرایه، اهداف متفاوتی را دنبال می‌کنند. تابع `np.arange()` آرایه‌هایی با اندازه‌های گام مشخص ایجاد می‌کند:
...
کتابخانه **pandas** قابلیت‌های قدرتمندی برای دستکاری داده‌ها فراهم می‌کند. DataFrameها ساختارهای داده‌ای برچسب‌دار دوبعدی هستند.

### خواندن داده‌ها: `pd.read_csv()` در برابر `pd.read_excel()`
...
*Matplotlib* کتابخانه رسم پایه برای Python است. این کتابخانه کنترل دقیقی بر ظاهر نمودار فراهم می‌کند.

### تابع `plt.plot()`

نمودارهای خطی پایه با استفاده از `plt.plot(x, y)` ایجاد می‌شوند:
...
پروژه [QuantEcon](https://quantecon.org) ابزارهای تخصصی برای مدل‌سازی اقتصادی ارائه می‌دهد. برای جزئیات بیشتر به [مستندات](https://quanteconpy.readthedocs.io/) مراجعه کنید.
...
مقادیر مورد انتظار $\mathbb{E}[X]$ نمایانگر پیامدهای میانگین هستند:
...
- **پرسش**: کدام سریع‌تر است: `numpy` یا پایتون معمولی؟
...
## "موارد لبه" و [ویژه] {نویسه‌ها}

این بخش نویسه‌های ویژه گوناگون را در عنوان‌ها آزمایش می‌کند.

### استفاده از `plt.subplot()` در `matplotlib` برای رسم چند نمودار
...
ابزارهای برنامه‌نویسی مانند **numpy**، *pandas* و `matplotlib` برای پژوهش‌های اقتصادی مدرن ضروری هستند. برای تحلیل کارآمد داده‌ها و مصورسازی نتایج، بر این کتابخانه‌ها تسلط پیدا کنید.
