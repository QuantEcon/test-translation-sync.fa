...
translation:
  ...
  headings:
    ...
    '*Matplotlib* Visualization Basics': اصول مصورسازی *Matplotlib*
    ...
    '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': استفاده از `matplotlib`'s `plt.subplot()` برای رسم چند نمودار
    Summary: خلاصه
---
...
کتابخانه `numpy` عملیات آرایه‌ای کارآمدی برای محاسبات عددی فراهم می‌کند. آرایه‌ها اساس محاسبات علمی در پایتون هستند.
...
روش‌های مختلف ایجاد آرایه اهداف متفاوتی را برآورده می‌کنند. تابع `np.arange()` آرایه‌هایی با اندازه گام مشخص ایجاد می‌کند:
...
مقادیر مورد انتظار $\mathbb{E}[X]$ نشان‌دهنده نتایج میانگین هستند:
...
این بخش نویسه‌های خاص گوناگون را در سرتیترها آزمایش می‌کند.

### استفاده از `matplotlib`'s `plt.subplot()` برای رسم چند نمودار
...
ابزارهای برنامه‌نویسی مانند **numpy**، *pandas*، و `matplotlib` برای پژوهش‌های اقتصادی مدرن ضروری هستند. برای تحلیل کارآمد داده‌ها و تجسم نتایج، بر این کتابخانه‌ها تسلط یابید.
